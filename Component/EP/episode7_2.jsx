...
export default function Ep07_Ch1() {
  ...
  async function getNextApperanceTime() {
    try {
      const docSnap1 = await getDoc(
        doc(dbService, "episode7_2h", "episode7_1")
      );
      const docSnap2 = await getDoc(
        doc(dbService, "episode7_2h", "episode7_2")
      );

      const docSnap3 = await getDoc(
        doc(dbService, "episode7_2h", "episode7_3")
      );
      const docSnap4 = await getDoc(
        doc(dbService, "episode7_2h", "episode7_4")
      );
  ...
  const EpContent = ({ id, areaName, bossName, time }) => {
    ...
    function notify() {
      toast(`에피소드 7(채널 2)의 ${bossName}의 필드 이벤트가 5분 남았어요!`, {
        limit: 1,
        autoClose: 300000,
