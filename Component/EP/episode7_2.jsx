...
export default function Ep07_Ch1() {
    ...
    // episode7_ch2의 collection Name
    const q = query(collection(dbService, "episode7_2h"));

    // 설정된 다음 시간을 받아오는 부분
    async function getNextApperanceTime() {
        try {

            const docSnap1 = await getDoc(doc(dbService, "episode7_2h", "episode7_1"));
            const docSnap2 = await getDoc(doc(dbService, "episode7_2h", "episode7_2"));

            const docSnap3 = await getDoc(doc(dbService, "episode7_2h", "episode7_3"));
            const docSnap4 = await getDoc(doc(dbService, "episode7_2h", "episode7_4"));
    ...
    const EpContent = ({ id, areaName, bossName, time }) => {
        ...
        function notify () {
            toast(`에피소드 7(채널 2)의 ${bossName}의 필드 이벤트가 10분 남았어요!`, { limit:1, autoClose:600000, pauseOnFocusLoss:false });
        }

        // 데이터베이스의 값 설정
        function setDBTime(newData, id) {
            return setDoc(doc(dbService, `episode7_2h`, `episode7_${id}`), newData, {
                merge: true,
            });
        }
