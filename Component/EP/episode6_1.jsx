...
export default function Ep06_Ch1() {
    ...
    const q = query(collection(dbService, "episode6"));

    // 설정된 다음 시간을 받아오는 부분
    async function getNextApperanceTime() {
        try {
            const docSnap1 = await getDoc(doc(dbService, "episode6", "episode6_1"));
            const docSnap2 = await getDoc(doc(dbService, "episode6", "episode6_2"));
            const docSnap3 = await getDoc(doc(dbService, "episode6", "episode6_3"));
            const docSnap4 = await getDoc(doc(dbService, "episode6", "episode6_4"));
            const docSnap5 = await getDoc(doc(dbService, "episode6", "episode6_5"));
    ...
    const EpContent = ({ id, areaName, bossName, time }) => {
        ...
        function notify () {
            toast(`에피소드 6(채널 1)의 ${bossName}의 필드 이벤트가 5분 남았어요!`, { limit:1, autoClose:300000, pauseOnFocusLoss:false });
        }

        // 데이터베이스의 값 설정
        function setDBTime(newData, id) {
            return setDoc(doc(dbService, `episode6`, `episode6_${id}`), newData, {
                merge: true,
            });
        }
