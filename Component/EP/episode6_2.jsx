...
import { ep06BossListAtom, userNameAtom } from "../../src/index";
...
export default function Ep06_Ch2() {
  ...
  async function getNextApperanceTime() {
    try {
      const docSnap1 = await getDoc(
        doc(dbService, "episode6_2ch", "episode6_1")
      );
      const docSnap2 = await getDoc(
        doc(dbService, "episode6_2ch", "episode6_2")
      );
      const docSnap3 = await getDoc(
        doc(dbService, "episode6_2ch", "episode6_3")
      );
      const docSnap4 = await getDoc(
        doc(dbService, "episode6_2ch", "episode6_4")
      );
      const docSnap5 = await getDoc(
        doc(dbService, "episode6_2ch", "episode6_5")
      );
  ...
  const EpContent = ({ id, areaName, bossName }) => {
    ...
    function notify() {
      toast(`에피소드 6(채널 2)의 ${bossName}의 필드 이벤트가 5분 남았어요!`, {
        limit: 1,
        autoClose: 300000,
