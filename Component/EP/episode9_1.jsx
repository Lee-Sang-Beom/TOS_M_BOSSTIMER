...
export default function Ep09_Ch1() {
  ...
  const q = query(collection(dbService, "episode9"));

  // 설정된 다음 시간을 받아오는 부분
  async function getNextApperanceTime() {
    try {
      const docSnap1 = await getDoc(doc(dbService, "episode9", "episode9_1"));
      const docSnap2 = await getDoc(doc(dbService, "episode9", "episode9_2"));
      const docSnap3 = await getDoc(doc(dbService, "episode9", "episode9_3"));
      const docSnap4 = await getDoc(doc(dbService, "episode9", "episode9_4"));
      const docSnap5 = await getDoc(doc(dbService, "episode9", "episode9_5"));
  ...
  const EpContent = ({ id, areaName, bossName }) => {
    ...
    function notify() {
      toast(`에피소드 9(채널 1)의 ${bossName}의 필드 이벤트가 5분 남았어요!`, {
        limit: 1,
        autoClose: 300000,
    ...
    function setDBTime(newData, id) {
      return setDoc(doc(dbService, `episode9`, `episode9_${id}`), newData, {
        merge: true,
      });
    }
    ...
    <section className="text-gray-600 body-font">
      <div className="px-5 py-24 mx-auto text-center">
        <div className="flex flex-col text-center w-full mb-20">
          <h1 className="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">{`EP09(채널 1)`}</h1>
          <p className="lg:w-1/3 mx-auto leading-relaxed text-base">{`에피소드9 (채널 1)의 필드보스 타이머입니다.`}</p>
        </div>
        <Segment>
