...
import { epEtcBossListAtom, userNameAtom } from "../../src/index";
...
export default function Ep_etc() {
  ...
  const [bossData, setBossData] = useRecoilState(epEtcBossListAtom);

  // episode6_ch1의 collection Name
  const q = query(collection(dbService, "episode_etc"));

  // 설정된 다음 시간을 받아오는 부분
  async function getNextApperanceTime() {
    try {
      const docSnap1 = await getDoc(
        doc(dbService, "episode_etc", "episode_etc_1")
      );
      const docSnap2 = await getDoc(
        doc(dbService, "episode_etc", "episode_etc_2")
      );
      const docSnap3 = await getDoc(
        doc(dbService, "episode_etc", "episode_etc_3")
      );
      const docSnap4 = await getDoc(
        doc(dbService, "episode_etc", "episode_etc_4")
      );
      const docSnap5 = await getDoc(
        doc(dbService, "episode_etc", "episode_etc_5")
      );
  ...
  const EpContent = ({ id, areaName, bossName }) => {
    ...
    function notify() {
      toast(`기타 항목으로 분류된 ${bossName}의 필드 이벤트가 5분 남았어요!`, {
        limit: 1,
        autoClose: 300000,
    ...
    function setDBTime(newData, id) {
      return setDoc(doc(dbService, `episode_etc`, `episode_etc_${id}`), newData, {
        merge: true,
      });
    }
    ...
    <section className="text-gray-600 body-font">
      <div className="px-5 py-24 mx-auto text-center">
        <div className="flex flex-col items-center text-center w-full mb-20">
          ...
          <h1 className="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">{`EP etc`}</h1>
          <p className="lg:w-1/3 mx-auto leading-relaxed text-base">{`변동성이 높은 지역을 대상으로 한, 기타 항목으로 분류된 필드보스 타이머입니다.`}</p>
        </div>
        <Segment>
