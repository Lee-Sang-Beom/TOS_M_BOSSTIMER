...
import { ep04BossListAtom } from "../../src/index"
...
export default function Ep04() {
    ...
    const [bossData, setBossData] = useRecoilState(ep04BossListAtom);

    // episode4의 collection Name
    const q = query(collection(dbService, "episode4"));

    // 설정된 다음 시간을 받아오는 부분
    async function getNextApperanceTime() {
        try {
            const docSnap1 = await getDoc(doc(dbService, "episode4", "episode4_1"));
            const docSnap2 = await getDoc(doc(dbService, "episode4", "episode4_2"));
            const docSnap3 = await getDoc(doc(dbService, "episode4", "episode4_3"));
            const docSnap4 = await getDoc(doc(dbService, "episode4", "episode4_4"));
    ...
    const EpContent = ({ id, areaName, bossName, time }) => {
        ...
        function notify () {
            toast(`에피소드 4의 ${bossName}의 필드 이벤트가 10분 남았어요!`, { limit:1, autoClose:600000, pauseOnFocusLoss:false });
        }

        // 데이터베이스의 값 설정
        function setDBTime(newData, id) {
            return setDoc(doc(dbService, `episode4`, `episode4_${id}`), newData, {
                merge: true,
            });
        }
        ...
        <section className="text-gray-600 body-font">
            <div className="px-5 py-24 mx-auto text-center">
                <div className="flex flex-col text-center w-full mb-20">
                    <h1 className="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">EP04</h1>
                    <p className="lg:w-1/3 mx-auto leading-relaxed text-base">에피소드 4의 필드보스 타이머입니다.</p>
                </div>
                <Segment>
