...
import { ep03BossListAtom } from "../../src/index"
import { useRecoilState } from "recoil";
import { toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';

export default function Ep03() {
    ...
    async function getNextApperanceTime() {
        try {
            const docSnap1 = await getDoc(doc(dbService, "episode3", "episode3_1"));
            const docSnap2 = await getDoc(doc(dbService, "episode3", "episode3_2"));
            const docSnap3 = await getDoc(doc(dbService, "episode3", "episode3_3"));
            const docSnap4 = await getDoc(doc(dbService, "episode3", "episode3_4"));
    ...
    const EpContent = ({ id, areaName, bossName, time }) => {
        ...
        function notify () {
            toast(`에피소드 3의 ${bossName}의 필드 이벤트가 10분 남았어요!`, { limit:1, autoClose:600000, pauseOnFocusLoss:false });
        }

        // 데이터베이스의 값 설정
        function setDBTime(newData, id) {
            return setDoc(doc(dbService, `episode3`, `episode3_${id}`), newData, {
                merge: true,
            });
        }
        ...
        <section className="text-gray-600 body-font">
            <div className="px-5 py-24 mx-auto text-center">
                <div className="flex flex-col text-center w-full mb-20">
                    <h1 className="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">EP03</h1>
                    <p className="lg:w-1/3 mx-auto leading-relaxed text-base">에피소드 3의 필드보스 타이머입니다.</p>
                </div>
                <Segment>
